...
//	const semLogContext = "update-event::string"
//
...
func parseUpdateOperationType(tok checkpoint.ResumeToken, m bson.M) (ChangeEvent, error) {
	...
	if err == nil {
		e.FullDocument, err = getDocument(m, "fullDocument", false)
	}
	...
	if err == nil {
		e.UpdateDescription, err = getDocument(m, "updateDescription", false)
	}
