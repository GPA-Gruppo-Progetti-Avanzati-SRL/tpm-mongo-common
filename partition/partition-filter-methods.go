...
/*
 * filter-int template: partitionNumber
 */

// AndPartitionNumberEqTo No Remarks
func (ca *Criteria) AndPartitionNumberEqTo(p int, nullValue ...int) *Criteria {
...
func (ca *Criteria) AndPartitionNumberGt(p int, nullValue ...int) *Criteria {
...
// @tpm-schematics:start-region("partition-number-field-filter-section")
// @tpm-schematics:end-region("partition-number-field-filter-section")
...
func (ca *Criteria) AndMongoCollection(p string) *Criteria {
	if p == "" {
		return ca
	}

	mName := fmt.Sprintf("mongo.collection")
	c := func() bson.E { return bson.E{Key: mName, Value: p} }
	*ca = append(*ca, c)
	return ca
}
