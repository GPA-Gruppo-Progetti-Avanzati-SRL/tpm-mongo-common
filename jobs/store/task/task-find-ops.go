...
func FindByPk(collection *mongo.Collection, domain, site, bidJob, bidTask string, mustFind bool, findOptions *options.FindOneOptionsBuilder) (*Task, bool, error) {
	// @tpm-schematics:end-region("find-by-pk-signature-section")
	const semLogContext = "task::find-by-pk"
	...
	f.Or().AndDomainEqTo(domain).AndSiteEqTo(site).AndEtEqTo(EType).AndJobIdEqTo(bidJob).AndBidEqTo(bidTask)
