...
type Task struct {
	...
	DataStreamType string                `json:"data_stream_type,omitempty" bson:"data_stream_type,omitempty" yaml:"data_stream_type,omitempty"`
...
func (s Task) IsZero() bool {
	return s.Bid == "" && s.Et == "" && s.Status == "" && s.Typ == "" && s.DataStreamType == "" && s.JobBid == "" && s.Info.IsZero() && len(s.Partitions) == 0
}
